...
let WishlistsController = class WishlistsController {
    ...
    async getWishlistById(wishId) {
        const wishlist = await this.wishlistsService.findWishlistById(Number(wishId));
        return wishlist;
    }
    async updateWishlist({ user }, wishId, dto) {
        return await this.wishlistsService.updateWishlist(Number(wishId), dto, user.id);
    }
    async deleteWishlist({ user }, id) {
